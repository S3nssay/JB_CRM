...
interface AgedDebtorsData {
  current: DebtorEntry[];
  days_1_30: DebtorEntry[];
  days_31_60: DebtorEntry[];
  days_61_90: DebtorEntry[];
  over_90: DebtorEntry[];
...
export default function AgedDebtors() {
  ...
  const { data, isLoading, error } = useQuery<AgedDebtorsData>({
    queryKey: ['/api/crm/accounting/reports/aged-debtors'],
  });
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-3">
        ...
        <p className="text-destructive font-medium">Failed to load aged debtors report</p>
  ...
  const bandTotals = AGING_BANDS.map((band) => {
    const entries = data[band.key as keyof AgedDebtorsData] as DebtorEntry[];
      ...
      <div className="flex items-center justify-between print:mb-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Outstanding amounts owed to you by customers, broken down by aging period
          </p>
        </div>
                ...
                <tbody>
                  {entries.map((entry) => (
                    <tr key={entry.id} className="border-b border-gray-100">
                      ...
                      <td className="py-1 pr-2">{entry.customer_name}</td>
